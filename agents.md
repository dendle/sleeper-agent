...
- **Messages/chat:** No new DMs, no new @mentions, no new league chat content since the 14:00 run (all visible chat entries are further-aged versions of already-known events).
- **Roster:** Braelon Allen (11576) and Omar Cooper (13276) still not synced to the roster API (known lag, syncs once the 3-round draft completes). `reserve: null`, taxi still just Baker (1/3 slots), FAAB $0 used.
- **Injuries:** Full sweep via Chrome JS fetch (all rostered player IDs), cross-checked `news_updated` timestamps against the 14:00 cutoff — zero players had newer news, nothing to evaluate. Existing Questionables unchanged: Hall (thigh), Pittman (leg), Worthy (shoulder), Estime (leg), Kraft (knee-ACL), Q. Johnston (lower body). No IR moves needed. David Montgomery's 13:20 news refresh from last run did not resurface with further detail — non-event.
