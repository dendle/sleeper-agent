...
- **Messages/chat:** No new DMs, no new @mentions, no new league chat content since the 15:03 run (all visible chat entries are further-aged versions of already-known events).
...
- **Injuries:** Full sweep via Chrome JS fetch (all rostered player IDs), cross-checked `news_updated` timestamps against the 15:03 cutoff — zero players had newer news, nothing to evaluate. Existing Questionables unchanged: Hall (thigh), Pittman (leg), Worthy (shoulder), Estime (leg), Kraft (knee-ACL), Q. Johnston (lower body). No IR moves needed. David Montgomery's 13:20 news refresh remains benign, no new detail.
